...
const addTodo = async (req, res) => {
  ...
  // validated data
  const { useremail, title, description, date_due, reminder } =
  ...
  try {
    await pool.execute(
      "INSERT INTO todos (id, useremail, title, description, reminder, date_due) VALUES(?,?,?,?,?,?)",
      insertMySQLdependencyArray
    );
    return res
      .status(201)
...
const updateTodo = async (req: { body: IUpdateTodoRequestBody }, res) => {
  // Joi schema validation
  const validationResult: ValidationResult<IUpdateTodoRequestBody> =
    await schemaUpdateTodo.validate(req.body);
  if (validationResult.error)
    return res
      .status(400)
      .json({
  ...
  // validated fields
  const { id, title, completed, description, date_due, reminder } =
    validationResult.value;
  ...
  Object.keys(validFields).forEach(
    (key) => validFields[key] === undefined && delete validFields[key]
  );
  ...
  try {
    await pool.execute(
      `UPDATE todos SET ${queryString} WHERE id=?`,
      queryArray
    );
    return res
      .status(200)
