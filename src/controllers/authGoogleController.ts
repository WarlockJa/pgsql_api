...
const authGoogleUser = async (req, res) => {
  ...
  if (!access_token)
    return res
      .status(400)
      .json({ message: "Access Token requried", status: 400 });
  ...
  try {
    ...
    if (Array.isArray(result[0]) && result[0].length === 0) {
      ...
      const hashedPassword = await bcrypt.hash(randomPassword, 10); // dummy password, to prevent unauthorized access
      ...
      await pool.execute(
        "INSERT INTO users (email, email_confirmed, name, surname, locale, password, refreshtoken, darkmode, authislocal) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          email,
          email_verified,
          idToken.name,
