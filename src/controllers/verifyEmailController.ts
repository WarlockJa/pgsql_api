...
const schemaVerifyUser = Joi.object<IVerifyUser>({
...
const verifyUser = async (req, res) => {
    ...
    try {
        ...
        const result = await pool.execute<OkPacket>('SELECT * FROM verify WHERE email = ?', [email]);
        // checking that verification request was issued
        // TODO confirm that will work with multiple outdated requests present
        if(Array.isArray(result[0]) && result[0].length === 0) return res.sendStatus(400);
        const verificationRequest: IVerifyDBRecord = result[0][0];
        // verifying token data
        if(verificationRequest.token !== token) return res.status(401).json({ message: 'Incorrect verification data' });
        ...
        await pool.execute('UPDATE users SET email_confirmed = ? WHERE email = ?', [true, email]);
