...
const logoutUser = async (req, res) => {
    ...
    try {
        const result = await pool.execute("UPDATE users SET refreshtoken = null WHERE email = ?", [userEmail]);
...
const confirmUser = async (req, res) => {
    ...
    const htmlVerificationLink = getEmailBody({
        h1: "Daily Planner",
        bodyText: 'You have received this e-mail as a part of verification process for <span style="color: #252525; font-weight: bold;">Daily Planner</span> website. Follow the link to confirm your e-mail address and unlock additional features, such as reminder option for your tasks!',
    ...
    const message = result.accepted?.length === 1
...
const deleteUser = async (req, res) => {
    ...
    try {
        ...
        if (result[0].affectedRows === 1) {
            ...
            await pool.execute("DELETE FROM todos WHERE useremail = ?", [
                email,
            ]);
