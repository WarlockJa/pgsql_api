...
const resetPassword = async (req, res) => {
    ...
    try {
        ...
        const result = await pool.execute("SELECT authislocal FROM users WHERE email = ?", [email]);
        ...
        try {
            ...
            await pool.execute("REPLACE INTO verify (email, token) VALUES(?, ?)", [email, emailConfirmationToken]);
        }
        catch (error) {
            return res
                .status(500)
        ...
        const htmlPasswordResetLink = getEmailBody({
            h1: "Daily Planner",
            bodyText: 'You have received this e-mail because someone, hopefully you, has requested a password reset for this e-mail on <span style="color: #252525; font-weight: bold;">Daily Planner</span> website. Follow the link to proceed with the password reset. If it wasn\' you it is safe to ignore this message.',
        ...
        const message = emailResult.accepted?.length === 1
